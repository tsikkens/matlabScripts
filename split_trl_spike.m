function varargout = split_trl_spike(varargin)
% split a trl matrix into baseline [-0.5 0] or response [0.2 0.7] trl's.
...
switch nargout
    ...
    offset = round(0.1*hdr.Fs*hdr.TimeStampPerSample);
    begsample = begsample + offset ;
    endsample = begsample + round(0.5*hdr.Fs*hdr.TimeStampPerSample);
